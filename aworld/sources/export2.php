...
function save(){
	...
	if($_POST['id_listz']!=""){
		$id_listx =" and mucdat=".$_POST['id_listz'];
	}else{
		$id_listx="";
	}
...
$objPHPExcel->setActiveSheetIndex( 0 )->setCellValue( 'A1', 'DANH SÁCH THƯỞNG  ( '.$batdau." đến ".$ketthuc." )" );
$objPHPExcel->setActiveSheetIndex( 0 )->setCellValue( 'A2', 'STT' )->setCellValue( 'B2', 'Họ tên' )->setCellValue( 'C2', 'Mã số' )->setCellValue( 'D2', 'Mức đạt' )->setCellValue( 'E2', 'Phần thưởng' )->setCellValue( 'F2', 'Ngày được thưởng' );
...
$sql = "select * from #_thuong where ngaytao >= ".$batdaux." and ngaytao <=".$ketthucx.$id_listx.$keywordx." order by id desc";
$d->query($sql);
$result=$d->result_array();	
$vitri=3;
for($i=0,$count_dmsp=count($result);$i<$count_dmsp;$i++) { 

        $sql11 = "select hoten,dienthoai,taikhoan from #_product where maso='".$result[$i]['maso']."'";
		$d->query($sql11);
		$hoten=$d->fetch_array();
		
			
	    $objPHPExcel->getActiveSheet()->getRowDimension($i+3)->setRowHeight(23);
    	$objPHPExcel->setActiveSheetIndex( 0 )->setCellValue( 'A'.$vitri,$i+1 )->setCellValue( 'B'.$vitri,$hoten['hoten'])->setCellValue( 'C'.$vitri,$result[$i]['maso'])->setCellValue( 'D'.$vitri,mucdat($result[$i]['mucdat']))->setCellValue( 'E'.$vitri,$result[$i]['phanthuong'])->setCellValue( 'F'.$vitri,date('d/m/Y H:i:s',$result[$i]['ngaytao']));	
...
// Rename sheet
$objPHPExcel->getActiveSheet()->setTitle('Danhsachthuong');
...
//Redirect output to a client’s web browser (Excel5)
      ...
      header( 'Content-Disposition: attachment;filename="Danhsachthuong_'.date('dmY').'.xls"' );
